...
register an endpoint and an event type; the service accepts events, delivers them asynchronously,
tracks delivery outcomes, and automatically retries failed deliveries with exponential backoff.
...
sections below describe not just what was built, but _why_, including decisions that were
...
2. **Send an event** — `POST /event` with an `event_type` and a `payload`. The service persists the
   event, looks up matching subscribers, and asynchronously delivers the event to each one via
   HTTP POST.
3. **Check delivery status** — `GET /events/{eventId}` returns the current delivery state for every
   subscriber the event fanned out to.
...
| `worker`   | Background polling loop that finds retries due for another attempt and triggers them via `delivery`.                                                  |
...
Every accepted event, persisted **synchronously**, before the async delivery goroutine ever
fires — so an event is never lost even if the process crashes immediately after returning `202`.
Persisted regardless of whether any subscriber currently exists for its event type (a deliberate
choice — an event is a fact that occurred, independent of who happened to be listening).
...
event_id       uuid
app_id         uuid
state          delivery_state  -- success | failed | retry_later | dead
error          text
attempt_count  int
...
timeline. Using Redis as the _sole_ store for delivery status was considered and rejected: it
...
**Sequential (not concurrent) delivery, in both the initial fan-out and the retry worker.** A
concurrent, goroutine-per-subscriber fan-out was considered and explicitly rejected for this
stage: it introduces unbounded outbound connections with no backpressure or pool limit, and solves
a scaling problem this project doesn't yet have. Logged as a natural Stage 4 (queueing/scaling)
concern rather than solved prematurely.

**Background worker as a goroutine within the same process, not a separate service.** A
`time.Ticker`-driven loop polls for due retries every 30 seconds, running inside the same binary
as the HTTP server. This keeps the deployment story simple (one process) while still cleanly
separating "orchestrate retries" (worker) from "execute one delivery attempt" (delivery) as
...
- **No queueing.** Delivery is triggered directly by the HTTP handler (`go delivery.DeliverEvent(...)`)
  rather than via a durable queue (e.g., Redis Streams). This is the natural next stage — it would
  decouple "accept the event" from "process the delivery," and enable multiple worker instances.
- **No concurrent delivery.** Both the initial fan-out and the retry worker process subscribers
  sequentially. Fine at small scale; would need a bounded worker pool at higher fan-out or retry
  volume.
...
- **Single worker, no distributed coordination.** If scaled to multiple worker instances, they
  would need a mechanism (row-level locking, a `processing` state) to avoid double-processing the
  same due retry — not needed with a single worker goroutine, as is currently the case.
